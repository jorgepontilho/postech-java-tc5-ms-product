...
COPY --from=build /app/target/*.jar /app/msproduct.jar

#CMD ["java", "-jar", "/app/msproduct.jar"]
...
CMD ["/bin/bash", "-c", "wait-for-it.sh -t 0 mysql-product:3307 -- java -jar msproduct.jar"]